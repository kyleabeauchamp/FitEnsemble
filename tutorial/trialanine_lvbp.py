...
measurements, predictions, uncertainties = example_loader.load_alanine()  # By default, subsample the data 5X to make the calculation faster.

# Load the backbone torsions and state assignments
phi, psi, assignments, indicators = example_loader.load_alanine_dihedrals()  # subsample the data 5X to make the calculation faster.
...
mu = lvbp_model.mcmc.trace("state_pops")[:].mean(0)
sigma = lvbp_model.mcmc.trace("state_pops")[:].std(0)
